...
print(">>>> process_image_router.py: SCRIPT EXECUTION STARTED (SIMPLE VERSION) <<<<")
from fastapi import APIRouter

print(">>>> process_image_router.py: FastAPI APIRouter imported. <<<<")

router = APIRouter()
print(">>>> process_image_router.py: APIRouter instance created. <<<<")

@router.get("/test-router") # 一个简单的测试路由
async def test_router_endpoint():
    print(">>>> process_image_router.py: /test-router endpoint was HIT! <<<<")
    return {"message": "Process image router (simple test) is alive!"}
print(">>>> process_image_router.py: /test-router endpoint defined. <<<<")

print(">>>> process_image_router.py: SCRIPT EXECUTION FINISHED (SIMPLE VERSION) <<<<")